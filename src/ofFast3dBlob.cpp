...
//                                CONSTRUCTORS
// ***************************************************************************
ofFast3dBlobDetector::ofFast3dBlobDetector() {
	m_pointCloud = NULL;
	kinectPtr = NULL;
	kWidth = 640;
	kHeight = 480;
...
//                                INIT
// ***************************************************************************
void ofFast3dBlobDetector::init(ofxKinectCommonBridge *newKinect) {
	kinectPtr = newKinect;
...
bool ofFast3dBlobDetector::findBlobs(ofImage * maskImage,
	...
	const int minPoints, const unsigned int maxBlobs)
{
	...
	std::vector<ofFast3dBlob*> tempBlobs;
	...
	while ((pixIndex < nPix) && (pixelsToProcess > 0) &&
		...
		if (lastQueued > minPoints) {
			...
			if ((blobVol >= minVol) && (blobVol <= maxVol)) {
				...
				ofFast3dBlob* newBlob = new ofFast3dBlob();
				...
				newBlob->contourIndices.push_back(pixIndex);
				...
				while (!isContourClosed) {
					...
					if (x > 0 && x < width && y > 0 && y < height){
						...
						if (m_pointCloud[neighbour].flag == numBlobs) {
							...
							newBlob->contourIndices.push_back(neighbour);
...
bool ofFast3dBlobDetector::createCloud(unsigned char * maskPix) {
	ofVec3f thePos;
...
// ************************************************************
void ofFast3dBlobDetector::setScale(const ofVec3f newScale) {
	scale = newScale;
}
...
// ************************************************************
ofVec3f ofFast3dBlobDetector::getScale() const {
	return scale;
};
...
// ************************************************************
bool ofFast3dBlobDetector::setResolution(ofFastBlobResolution newResolution) {
...
// ************************************************************
enum ofFastBlobResolution ofFast3dBlobDetector::getResolution() {
	return resolution;
}
...
//
// ************************************************************
bool ofFast3dBlobDetector::isInited() {

...
namespace garment {
	...
	FlyingLights::FlyingLights() : k_num_of_lights_(20) {
		lights_.reserve(k_num_of_lights_);
		...
		// Possibly read the frames from a GIF image
		/*
		...
		for (int i = 1; i <= 50; ++i) {
			video_frames_images_.push_back(ofImage());
			...
			video_frames_images_.back().loadImage(file_name.str());
	...
	void FlyingLights::Update(const std::vector<Fold> &folds) {
		...
		// Generate new particles from the folds
		if (!folds.empty()) {
			int living_particles_num = lights_.size();
			float light_radius_variation = light_radius_ * 0.5;
			for (int i = 0; i < k_num_of_lights_ - living_particles_num; ++i) {
				...
				lights_.back().InitUVTex(ofRandom(light_radius_ - light_radius_variation, light_radius_ + light_radius_variation), video_frames_images_[0].getTextureReference());
			}
		}
	...
	void FlyingLights::Draw() {
		...
		for (int i = 0; i < lights_.size(); ++i) {
			lights_[i].Draw(video_frames_images_, video_fps_ * last_frame_time());
		}

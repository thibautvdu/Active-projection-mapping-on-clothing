...
using mrpt::math::TLine3D;
	...
	void Ransac3DlineDistance(const CMatrixDouble &allData, const std::vector< CMatrixDouble > & testModels, const double distanceThreshold,
	...
	inline bool Ransac3DlineDegenerate( const CMatrixDouble &all_data, const mrpt::vector_size_t &use_indices) {
		MRPT_UNUSED_PARAM(all_data);
		MRPT_UNUSED_PARAM(use_indices);
		return false;
	}

	void RansacDetect3Dlines(const std::vector<ofVec3f> &point_cloud, std::vector<std::pair<size_t, TLine3D> > &out_detected_lines,
